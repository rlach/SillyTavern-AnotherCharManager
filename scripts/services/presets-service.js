...
export function getPreset(presetIndex) {
    if (presetIndex < 0 || presetIndex >= acm.settings.getSetting('dropdownPresets').length) {
        throw new Error('Invalid preset index');
    }
    return { ...acm.settings.getSetting('dropdownPresets')[presetIndex] };
}
...
export function updatePresetName(presetIndex, name) {
    ...
    const updatedPresets = [...acm.settings.getSetting('dropdownPresets')];
    updatedPresets[presetIndex] = {
        ...updatedPresets[presetIndex],
        name: name,
    };
    acm.settings.updateSetting('dropdownPresets', updatedPresets);
}
